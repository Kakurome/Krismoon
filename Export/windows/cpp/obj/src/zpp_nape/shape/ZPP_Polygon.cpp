...
{
		HX_STACK_FRAME("zpp_nape.shape.ZPP_Polygon","lverts_pa_immutable",0x49082076,"zpp_nape.shape.ZPP_Polygon.lverts_pa_immutable","zpp_nape/shape/Polygon.hx",190,0x571b0a14)
		HX_STACK_THIS(this)
		HX_STACK_LINE(190)
		if (((bool((bool((this->body != null())) && bool((this->body->type == ::zpp_nape::util::ZPP_Flags_obj::id_BodyType_STATIC)))) && bool((this->body->space != null()))))){
			HX_STACK_LINE(190)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Cannot modify local vertex of Polygon added to a static body whilst within a Space"));
		}
	}
return null();
}
...
Void ZPP_Polygon_obj::gverts_pa_validate( ){
{
		...
		if (((this->body == null()))){
			HX_STACK_LINE(195)
			HX_STACK_DO_THROW(HX_CSTRING("Error: World vertex only makes sense when Polygon is contained in a rigid body"));
		}
		HX_STACK_LINE(197)
		if ((this->zip_gverts)){
			HX_STACK_LINE(197)
...
Void ZPP_Polygon_obj::__validate_angDrag( ){
{
		...
		if (((this->lverts->length < (int)3))){
			HX_STACK_LINE(1395)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Polygon's with less than 3 vertices have no meaningful angDrag"));
		}
		HX_STACK_LINE(1397)
		this->validate_area_inertia();
		HX_STACK_LINE(1398)
...
Void ZPP_Polygon_obj::localCOM_validate( ){
{
		...
		if (((this->lverts->next == null()))){
			HX_STACK_LINE(1770)
			HX_STACK_DO_THROW(HX_CSTRING("Error: An empty polygon does not have any meaningful localCOM"));
		}
		HX_STACK_LINE(1772)
