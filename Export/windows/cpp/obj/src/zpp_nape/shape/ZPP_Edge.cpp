...
Void ZPP_Edge_obj::lnorm_validate( ){
{
		...
		if (((this->polygon == null()))){
			HX_STACK_LINE(237)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Edge not currently in use"));
		}
		HX_STACK_LINE(239)
...
Void ZPP_Edge_obj::gnorm_validate( ){
{
		...
		if (((this->polygon->body == null()))){
			HX_STACK_LINE(264)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Edge worldNormal only makes sense if the parent Polygon is contained within a rigid body"));
		}
		HX_STACK_LINE(266)
		{
			HX_STACK_LINE(266)
