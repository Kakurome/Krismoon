...
{
		HX_STACK_FRAME("zpp_nape.shape.ZPP_Circle","localCOM_immutable",0x9d2e3a44,"zpp_nape.shape.ZPP_Circle.localCOM_immutable","zpp_nape/shape/Circle.hx",235,0x3e895b1e)
		HX_STACK_THIS(this)
		HX_STACK_LINE(235)
		if (((bool((bool((this->body != null())) && bool((this->body->type == ::zpp_nape::util::ZPP_Flags_obj::id_BodyType_STATIC)))) && bool((this->body->space != null()))))){
			HX_STACK_LINE(235)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Cannot modify localCOM of Circle added to a static Body whilst within a Space"));
		}
	}
return null();
}
