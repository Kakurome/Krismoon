	...
	HX_STACK_LINE(212)
	{
		HX_STACK_LINE(212)
		{
			...
			if (((bool((bool((this->zpp_inner->body != null())) && bool((this->zpp_inner->body->type == ::zpp_nape::util::ZPP_Flags_obj::id_BodyType_STATIC)))) && bool((this->zpp_inner->body->space != null()))))){
				HX_STACK_LINE(212)
				HX_STACK_DO_THROW(HX_CSTRING("Error: Cannot modifiy radius of Circle contained in static object once added to space"));
			}
			HX_STACK_LINE(212)
			if (((radius != this->zpp_inner_zn->radius))){
				...
				if (((radius < ::nape::Config_obj::epsilon))){
					HX_STACK_LINE(212)
					HX_STACK_DO_THROW(((HX_CSTRING("Error: Circle::radius (") + radius) + HX_CSTRING(") must be > Config.epsilon")));
				}
				HX_STACK_LINE(212)
				if (((radius > 1e100))){
					HX_STACK_LINE(212)
					HX_STACK_DO_THROW(((HX_CSTRING("Error: Circle::radius (") + radius) + HX_CSTRING(") must be < PR(Const).FMAX")));
				}
				HX_STACK_LINE(212)
