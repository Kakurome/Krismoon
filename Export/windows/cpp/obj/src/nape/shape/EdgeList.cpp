...
::nape::shape::Edge EdgeList_obj::at( int index){
	...
	struct _Function_1_1{
		inline static bool Block( hx::ObjectPtr< ::nape::shape::EdgeList_obj > __this,int &index){
			HX_STACK_FRAME("*","closure",0x5bdab937,"*.closure","nape/shape/EdgeList.hx",368,0x6c4938ee)
			{
				HX_STACK_LINE(368)
	...
	if ((this->zpp_inner->reverse_flag)){
		...
		if (((index == _g2))){
			...
			HX_STACK_LINE(370)
			{
				...
				HX_STACK_LINE(370)
				_g3 = this->zpp_inner->user_length;
			}
			HX_STACK_LINE(370)
			int _g4 = (_g3 - (int)2);		HX_STACK_VAR(_g4,"_g4");
			HX_STACK_LINE(370)
			_g5 = (_g4 - index);
		}
		HX_STACK_LINE(370)
	...
	if (((bool((index < this->zpp_inner->at_index)) || bool((this->zpp_inner->at_ite == null()))))){
		...
		HX_STACK_LINE(374)
		::zpp_nape::util::ZNPNode_ZPP_Edge _g6 = this->zpp_inner->inner->iterator_at(index);		HX_STACK_VAR(_g6,"_g6");
		HX_STACK_LINE(374)
		this->zpp_inner->at_ite = _g6;
	}
	else{
		HX_STACK_LINE(377)
		while((true)){
...
bool EdgeList_obj::push( ::nape::shape::Edge obj){
	...
	if ((this->zpp_inner->immutable)){
		HX_STACK_LINE(418)
		HX_STACK_DO_THROW(HX_CSTRING("Error: EdgeList is immutable"));
	}
	HX_STACK_LINE(420)
	...
	HX_STACK_ARG(deep,"deep")
{
		...
		HX_STACK_LINE(664)
		{
			...
			while((true)){
				...
				if ((deep)){
					HX_STACK_LINE(666)
					HX_STACK_DO_THROW(HX_CSTRING("Error: Edge is not a copyable type"));
					HX_STACK_LINE(668)
					_g1 = null();
				}
				else{
