...
bool velocityOnly = __o_velocityOnly.Default(false);
{
	...
	if (((dimensions < (int)1))){
		HX_STACK_LINE(379)
		HX_STACK_DO_THROW(HX_CSTRING("Error: Constraint dimension must be at least 1"));
	}
	HX_STACK_LINE(382)
...
::nape::constraint::UserConstraint UserConstraint_obj::__copy( ){
	...
	HX_STACK_THIS(this)
	HX_STACK_LINE(243)
	HX_STACK_DO_THROW(HX_CSTRING("Error: UserConstraint::__copy must be overriden"));
	HX_STACK_LINE(245)
	return null();
}
...
Void UserConstraint_obj::__position( Array< Float > err){
{
		...
		HX_STACK_LINE(302)
		HX_STACK_DO_THROW(HX_CSTRING("Error: UserConstraint::__position must be overriden"));
	}
return null();
}
...
Void UserConstraint_obj::__velocity( Array< Float > err){
{
		...
		HX_STACK_ARG(err,"err")
		HX_STACK_LINE(315)
		HX_STACK_DO_THROW(HX_CSTRING("Error: Userconstraint::__velocity must be overriden"));
	}
return null();
}
...
Void UserConstraint_obj::__eff_mass( Array< Float > eff){
{
		...
		HX_STACK_LINE(337)
		HX_STACK_DO_THROW(HX_CSTRING("Error: UserConstraint::__eff_mass must be overriden"));
	}
return null();
}
...
Void UserConstraint_obj::__impulse( Array< Float > imp,::nape::phys::Body body,::nape::geom::Vec3 out){
{
		...
		HX_STACK_LINE(360)
		HX_STACK_DO_THROW(HX_CSTRING("Error: UserConstraint::__impulse must be overriden"));
	}
return null();
}
...
::nape::geom::Vec3 UserConstraint_obj::bodyImpulse( ::nape::phys::Body body){
	HX_STACK_FRAME("nape.constraint.UserConstraint","bodyImpulse",0xdad439d8,"nape.constraint.UserConstraint.bodyImpulse","nape/constraint/UserConstraint.hx",411,0x5f14de6b)
	HX_STACK_THIS(this)
	HX_STACK_ARG(body,"body")
	HX_STACK_LINE(413)
	if (((body == null()))){
	...
	if (((oldBody != newBody))){
		...
		if (((oldBody != null()))){
			...
			if ((!(this->zpp_inner_zn->remBody(oldBody->zpp_inner)))){
				HX_STACK_LINE(510)
				HX_STACK_DO_THROW(HX_CSTRING("Error: oldBody is not registered to the cosntraint"));
			}
			HX_STACK_LINE(513)
			if (((bool(this->zpp_inner->active) && bool((((  (((this->zpp_inner->space == null()))) ? ::nape::space::Space(null()) : ::nape::space::Space(this->zpp_inner->space->outer) )) != null()))))){
				HX_STACK_LINE(514)
