...
::nape::dynamics::Arbiter ArbiterList_obj::at( int index){
	...
	struct _Function_1_1{
		inline static bool Block( hx::ObjectPtr< ::nape::dynamics::ArbiterList_obj > __this,int &index){
			HX_STACK_FRAME("*","closure",0x5bdab937,"*.closure","nape/dynamics/ArbiterList.hx",368,0xfd301dcb)
			{
				HX_STACK_LINE(368)
	...
	HX_STACK_ARG(deep,"deep")
{
		...
		HX_STACK_LINE(664)
		{
			...
			while((true)){
				...
				if ((deep)){
					HX_STACK_LINE(666)
					HX_STACK_DO_THROW(HX_CSTRING("Error: Arbiter is not a copyable type"));
					HX_STACK_LINE(668)
					_g1 = null();
				}
				else{
