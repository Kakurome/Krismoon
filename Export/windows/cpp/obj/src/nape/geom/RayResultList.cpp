...
::nape::geom::RayResult RayResultList_obj::at( int index){
	...
	struct _Function_1_1{
		inline static bool Block( hx::ObjectPtr< ::nape::geom::RayResultList_obj > __this,int &index){
			HX_STACK_FRAME("*","closure",0x5bdab937,"*.closure","nape/geom/RayResultList.hx",368,0x8086abe5)
			{
				HX_STACK_LINE(368)
	...
	if (((bool((index < this->zpp_inner->at_index)) || bool((this->zpp_inner->at_ite == null()))))){
		...
		HX_STACK_LINE(374)
		::zpp_nape::util::ZNPNode_RayResult _g4 = this->zpp_inner->inner->iterator_at(index);		HX_STACK_VAR(_g4,"_g4");
		HX_STACK_LINE(374)
...
bool RayResultList_obj::push( ::nape::geom::RayResult obj){
	...
	if ((this->zpp_inner->immutable)){
		HX_STACK_LINE(418)
		HX_STACK_DO_THROW(HX_CSTRING("Error: RayResultList is immutable"));
	}
	HX_STACK_LINE(420)
	...
	HX_STACK_ARG(deep,"deep")
{
		...
		HX_STACK_LINE(664)
		{
			...
			while((true)){
				...
				if ((deep)){
					HX_STACK_LINE(666)
					HX_STACK_DO_THROW(HX_CSTRING("Error: RayResult is not a copyable type"));
					HX_STACK_LINE(668)
					_g1 = null();
				}
				else{
