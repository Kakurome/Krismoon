...
Void ZPP_Body_obj::pos_invalidate( ::zpp_nape::geom::ZPP_Vec2 pos){
{
		...
		if (((bool((this->type == ::zpp_nape::util::ZPP_Flags_obj::id_BodyType_STATIC)) && bool((this->space != null()))))){
			HX_STACK_LINE(487)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Cannot move a static object once inside a Space"));
		}
		HX_STACK_LINE(489)
...
Void ZPP_Body_obj::vel_invalidate( ::zpp_nape::geom::ZPP_Vec2 vel){
{
		...
		if (((this->type == ::zpp_nape::util::ZPP_Flags_obj::id_BodyType_STATIC))){
			HX_STACK_LINE(538)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Static body cannot have its velocity set."));
		}
		HX_STACK_LINE(540)
		{
			HX_STACK_LINE(541)
...
Void ZPP_Body_obj::force_invalidate( ::zpp_nape::geom::ZPP_Vec2 force){
{
		...
		if ((!(((this->type == ::zpp_nape::util::ZPP_Flags_obj::id_BodyType_DYNAMIC))))){
			HX_STACK_LINE(676)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Non-dynamic body cannot have force applied."));
		}
		HX_STACK_LINE(678)
		{
			HX_STACK_LINE(679)
...
Void ZPP_Body_obj::getlocalCOM( ){
{
		...
		if (((this->shapes->head == null()))){
			HX_STACK_LINE(1191)
			HX_STACK_DO_THROW(HX_CSTRING("Error: localCOM only makes sense when Body has Shapes"));
		}
		HX_STACK_LINE(1193)
		this->validate_localCOM();
	}
...
Void ZPP_Body_obj::getworldCOM( ){
{
		...
		if (((this->shapes->head == null()))){
			HX_STACK_LINE(1197)
			HX_STACK_DO_THROW(HX_CSTRING("Error: worldCOM only makes sense when Body has Shapes"));
		}
		HX_STACK_LINE(1199)
...
{
		HX_STACK_FRAME("zpp_nape.phys.ZPP_Body","__immutable_midstep",0x56f8ba0b,"zpp_nape.phys.ZPP_Body.__immutable_midstep","zpp_nape/phys/Body.hx",1205,0x4bb115c1)
		HX_STACK_THIS(this)
...
Void ZPP_Body_obj::clear( ){
{
		...
		if (((this->space != null()))){
			HX_STACK_LINE(1210)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Cannot clear a Body if it is currently being used by a Space!"));
		}
		HX_STACK_LINE(1213)
		if ((!(((this->constraints->head == null()))))){
			HX_STACK_LINE(1213)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Cannot clear a Body if it is currently being used by a constraint!"));
		}
		HX_STACK_LINE(1216)
		while((true)){
			HX_STACK_LINE(1216)
...
Void ZPP_Body_obj::aabb_validate( ){
{
		...
		if (((this->shapes->head == null()))){
			HX_STACK_LINE(1388)
			HX_STACK_DO_THROW(HX_CSTRING("Error: bounds only makes sense when Body has shapes"));
		}
		HX_STACK_LINE(1390)
		{
			HX_STACK_LINE(1390)
...
Void ZPP_Body_obj::shapes_modifiable( ){
{
		...
		if (((bool((this->type == ::zpp_nape::util::ZPP_Flags_obj::id_BodyType_STATIC)) && bool((this->space != null()))))){
			HX_STACK_LINE(1417)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Cannot modifiy shapes of static object once added to Space"));
		}
	}
return null();
}
