...
Float viscosity = __o_viscosity.Default(1);
{
	...
	HX_STACK_LINE(241)
	{
		...
		if (((density != (this->zpp_inner->density * (int)1000)))){
			...
			if (((density != density))){
				HX_STACK_LINE(241)
				HX_STACK_DO_THROW(HX_CSTRING("Error: FluidProperties::density cannot be NaN"));
			}
			HX_STACK_LINE(241)
	...
	HX_STACK_LINE(242)
	{
		...
		if (((viscosity != this->zpp_inner->viscosity))){
			...
			if (((viscosity != viscosity))){
				HX_STACK_LINE(242)
				HX_STACK_DO_THROW(HX_CSTRING("Error: FluidProperties::viscosity cannot be NaN"));
			}
			HX_STACK_LINE(242)
			if (((viscosity < (int)0))){
				HX_STACK_LINE(242)
				HX_STACK_DO_THROW(((HX_CSTRING("Error: FluidProperties::viscosity (") + viscosity) + HX_CSTRING(") must be >= 0")));
			}
			HX_STACK_LINE(242)
