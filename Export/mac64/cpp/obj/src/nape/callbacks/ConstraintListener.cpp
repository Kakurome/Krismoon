...
int precedence = __o_precedence.Default(0);
{
	...
	if (((handler == null()))){
		HX_STACK_LINE(258)
		HX_STACK_DO_THROW(HX_CSTRING("Error: ConstraintListener::handler cannot be null"));
	}
	HX_STACK_LINE(261)
	...
	if (((event == _g1))){
		...
		if (((event == _g3))){
			...
			if (((event == _g5))){
				...
				HX_STACK_LINE(267)
				HX_STACK_DO_THROW((_g7 + HX_CSTRING("' is not a valid event type for a ConstraintListener")));
			}
		}
	}
	HX_STACK_LINE(270)
	::nape::callbacks::OptionType _g8 = ::zpp_nape::callbacks::ZPP_OptionType_obj::argument(options);		HX_STACK_VAR(_g8,"_g8");
	HX_STACK_LINE(270)
	::zpp_nape::callbacks::ZPP_ConstraintListener _g9 = ::zpp_nape::callbacks::ZPP_ConstraintListener_obj::__new(_g8,xevent,handler);		HX_STACK_VAR(_g9,"_g9");
	HX_STACK_LINE(270)
	this->zpp_inner_zn = _g9;
	HX_STACK_LINE(271)
