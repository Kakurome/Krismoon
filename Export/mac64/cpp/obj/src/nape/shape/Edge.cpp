...
HX_STACK_THIS(this)
{
	...
	if ((!(::zpp_nape::shape::ZPP_Edge_obj::internal))){
		HX_STACK_LINE(187)
		HX_STACK_DO_THROW(HX_CSTRING("Error: Cannot instantiate an Edge derp!"));
	}
}
;
	return null();
...
::nape::shape::Polygon Edge_obj::get_polygon( ){
	...
	if (((this->zpp_inner->polygon == null()))){
		HX_STACK_LINE(197)
		HX_STACK_DO_THROW(HX_CSTRING("Error: Edge not current in use"));
	}
	HX_STACK_LINE(199)
...
Float Edge_obj::get_worldProjection( ){
	...
	if (((this->zpp_inner->polygon->body == null()))){
		HX_STACK_LINE(265)
		HX_STACK_DO_THROW(HX_CSTRING("Error: Edge world projection only makes sense for Polygons contained within a rigid body"));
	}
	HX_STACK_LINE(267)
	{
		HX_STACK_LINE(267)
