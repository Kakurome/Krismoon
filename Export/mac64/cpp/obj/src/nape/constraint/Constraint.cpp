...
::nape::space::Space Constraint_obj::set_space( ::nape::space::Space space){
	...
	HX_STACK_LINE(252)
	{
		...
		if (((this->zpp_inner->compound != null()))){
			HX_STACK_LINE(255)
			HX_STACK_DO_THROW(HX_CSTRING("Error: Cannot set the space of a Constraint belonging to a Compound, only the root Compound space can be set"));
		}
		HX_STACK_LINE(258)
...
bool Constraint_obj::get_isSleeping( ){
	...
	if (((bool((this->zpp_inner->space == null())) || bool(!(this->zpp_inner->active))))){
		HX_STACK_LINE(293)
		HX_STACK_DO_THROW(HX_CSTRING("Error: isSleeping only makes sense if constraint is active and inside a space"));
	}
	HX_STACK_LINE(296)
