...
::nape::constraint::Constraint ConstraintList_obj::at( int index){
	...
	struct _Function_1_1{
		inline static bool Block( hx::ObjectPtr< ::nape::constraint::ConstraintList_obj > __this,int &index){
			HX_STACK_FRAME("*","closure",0x5bdab937,"*.closure","nape/constraint/ConstraintList.hx",368,0x16122b78)
			{
				HX_STACK_LINE(368)
	...
	if (((bool((index < this->zpp_inner->at_index)) || bool((this->zpp_inner->at_ite == null()))))){
		...
		HX_STACK_LINE(374)
		::zpp_nape::util::ZNPNode_ZPP_Constraint _g4 = this->zpp_inner->inner->iterator_at(index);		HX_STACK_VAR(_g4,"_g4");
		HX_STACK_LINE(374)
...
bool ConstraintList_obj::push( ::nape::constraint::Constraint obj){
	...
	if ((this->zpp_inner->immutable)){
		HX_STACK_LINE(418)
		HX_STACK_DO_THROW(HX_CSTRING("Error: ConstraintList is immutable"));
	}
	HX_STACK_LINE(420)
	...
	HX_STACK_ARG(deep,"deep")
{
		...
		HX_STACK_LINE(664)
		{
			...
			while((true)){
				...
				if ((deep)){
					HX_STACK_LINE(666)
					HX_STACK_DO_THROW(HX_CSTRING("Error: Constraint is not a copyable type"));
					HX_STACK_LINE(668)
					_g1 = null();
				}
				else{
