...
Float ratio = __o_ratio.Default(1.0);
{
	...
	HX_STACK_LINE(341)
	{
		HX_STACK_LINE(341)
		{
			...
			if (((rate != rate))){
				HX_STACK_LINE(341)
				HX_STACK_DO_THROW(HX_CSTRING("Error: MotorJoint::rate cannot be NaN"));
			}
			HX_STACK_LINE(341)
	...
	HX_STACK_LINE(342)
	{
		HX_STACK_LINE(342)
		{
			...
			if (((ratio != ratio))){
				HX_STACK_LINE(342)
				HX_STACK_DO_THROW(HX_CSTRING("Error: MotorJoint::ratio cannot be NaN"));
			}
			HX_STACK_LINE(342)
...
::nape::geom::Vec3 MotorJoint_obj::bodyImpulse( ::nape::phys::Body body){
	HX_STACK_FRAME("nape.constraint.MotorJoint","bodyImpulse",0x6d4cc5e5,"nape.constraint.MotorJoint.bodyImpulse","nape/constraint/MotorJoint.hx",359,0x6a1b57fe)
	HX_STACK_THIS(this)
